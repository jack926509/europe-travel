...
## 2026-09-08 — 介面與閱讀體驗優化

- 章節導覽：三個章節以上的頁面自動產生「這一頁」跳轉列；路線頁加上城市快速跳轉。
- 主導覽在窄螢幕會自動捲到目前頁面，兩側加上漸層提示還有其他項目。
- 寬表格改為首欄固定並顯示可捲動提示，欄寬保留可讀寬度，可用鍵盤捲動。
- 搜尋加入輸入防抖與中文組字處理、命中字詞標示、Enter 開啟第一筆、「／」聚焦與 Esc 清除。
- 卡片加入 hover／focus 回饋，站內卡片整張可點；長頁面新增回到頂端按鈕。
- 顏色集中為 `:root` 的語意 token；表單框線改用對比達 3:1 的色票，導覽焦點外框改為淺色，在深藍底上可辨識。
- 補上 canonical 與 Open Graph／Twitter 分享標記，首圖加上載入優先度提示。
- 首圖改以 `<picture>` 提供 WebP，瀏覽器不支援時仍取原本的 JPEG：
  2400×543／quality 80，317 KB，較原檔 1.14 MB 少約 73%；版面顯示寬度最大
  1160 px，仍保有 2 倍畫質。

## 2026-09-08 — 旅遊資料校正與城市規劃補強

- 依官方來源更新 2026 巴黎、巴塞隆納、里斯本與波多交通票價及使用限制。
- 修正法國、西班牙與葡萄牙旅客 VAT 退稅門檻，以及「離開歐盟出口點」的驗證原則。
- 修正巴黎迪士尼門票退改、Single Rider、遊行地點、Extra Magic Time 與雙園夜間安排。
- 補充 Lisboa Card 現行價格、實體卡兌換、指定時段與暫停開放快照。
- 釐清聖家堂 2026 耶穌基督塔里程碑，移除整座教堂完工的錯誤說法。
- 移除航班、列車、店家與品牌的固定低價、固定開賣窗口及未查證價差。
- 擴充住宿區域、南法無車玩法、波爾多、瓦倫西亞、杜羅河谷、阿爾加維及 16 個路線模組。
- 新增 `docs/SOURCES.md`，記錄官方來源、適用範圍、查核日與後續重查時機。

## 2026-09-08 — 三專案整合

- 以 europe-travel 為主專案，加入巴黎迪士尼獨立專題與 CDG-BCN 城市模組。
- 改為 11 個共用導覽頁面，新增全文搜尋與 GitHub 資料收集表單。
- 移除固定出發日、日期限定路線與舊版法規預測，保留季節及查核日期。
- 擴充法國全國與西葡目的地入口；原筆記標示待複查。
- 建立可重複產生頁面的建置程式與來源映射文件。
- 已核對 Disney Adventure World 名稱與官方開放事件；不宣稱全部原資料為最新。
